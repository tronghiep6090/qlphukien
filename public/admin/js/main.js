...
$(function () {
    ...
    $(document).on('click', '.delete-item-product', function () {
        ...
        var _that = $(this);
         if ( totalRowCount > 2 ) {
             ...
             $.confirm({
                 title: 'Thông báo',
                 content: 'Dữ liệu nhập hàng cần có ít nhất một hàng hóa',
                 type: 'green',
                 buttons: {
                     ok: {
                         text: "ok!",
    ...
    function calculateTotalAmount(_that, activePrice = null) {
        ...
        if ( totalNumber < 0) {
            message = "Số lượng hàng hóa không được phép nhỏ hơn 0."
        }
        ...
        $.ajax({
            ...
            if (result.flgCheck) {
                ...
                var stringNotification = 'Số lượng hàng hóa không đủ. Số lượng còn lại ' + result.number;
