...
@if (!empty($productExcels))
    @foreach($productExcels as $keys => $productExcel)
        <tr location="{{ $keys }}" class="product_{{ $keys }}">
            ...
            <td>
                <input type="number" class="form-control pw_total_number" name="pw_total_number[{{ $keys }}]" value="{!! $productExcel['total_number'] !!}" placeholder="Số lượng hàng hóa" required>
            </td>
            <td>
                ...
                <input type="number" class="form-control pw_custom_price" name="pw_custom_price[{{ $keys }}]" value="{!! $productExcel['custom_price'] !!}" placeholder="Giá hàng hóa" style="display: {{ $productExcel['active_price'] == 4 ? 'block' : 'none' }}">
            </td>
